...
export class SimpleConclusionComponent implements OnInit {
  ...
  isUdbytte() {
    return (
      this.state.aktier.type === "udbytte" &&
  ...
  noShowForUdbytte() {
    const specialCountries = ["BM", "KY", "VG", "GG", "IM", "JE"],
  ...
  idNameTxt() {
    return this.text.get(`dobbeltRenteSats_Land_${this.state.mainState.land}`);
  }
  ...
  dobbeltRenteSatsLande() {
    const country = this.state.mainState.land;

    return ["IN", "MX"].indexOf(country) > -1;
  }

  isRente() {
    return (
      this.state.mainState.type === "capitalIncome" &&
      this.common.getCountryGroup() !== 8 &&
  ...
  getUdbyttePct() {
    ...
    const specialCountry = pctMap.find(
      el => el.country === this.state.mainState.land
    );
  ...
  getRentePct() {
    ...
    const sats = Object.keys(satser).find(el => {
      return satser[el].indexOf(country) > -1;
    });

    return sats ? (sats === "special12" ? 12.5 : sats) : 0;
  }
}
